...
var _adminBro = _interopRequireWildcard(require("../../../admin-bro"));
...
const stubAdminBro = () => {
  const stubbedAdmin = _sinon.default.createStubInstance(_adminBro.default);
  ...
  return Object.assign(stubbedAdmin, {
    ...
    options: _objectSpread(_objectSpread({}, _adminBro.defaultOptions), {}, {
      rootPath: '/admin'
    })
  });
};
...
describe('ResourceDecorator', function () {
  ...
  describe('#getPropertyByKey', function () {
    ...
    it('returns nested property under mixed', function () {
      ...
      (0, _chai.expect)(decoratedProperty.path).to.eq(path);
    });
    ...
    it('returns property when it is an array', function () {
      ...
      const decoratedProperty = decorator.getPropertyByKey(path);
      (0, _chai.expect)(decoratedProperty).to.be.an.instanceof(_propertyDecorator.default);
      (0, _chai.expect)(decoratedProperty.path).to.eq(arrayProperty.path());
    });
    it('returns property when it is an nested array', function () {
      ...
      const decoratedProperty = decorator.getPropertyByKey(path);
      (0, _chai.expect)(decoratedProperty).to.be.an.instanceof(_propertyDecorator.default);
      (0, _chai.expect)(decoratedProperty.path).to.eq([arrayProperty.path(), nested1Property.path()].join('.'));
    });
  });
